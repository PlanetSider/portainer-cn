...
describe('ResizeClaimEditForm', () => {
  ...
  it('renders the form with the claim name in the title and pre-filled size', async () => {
    ...
    expect(
      await screen.findByText('调整持久卷声明容量')
    ).toBeVisible();
    expect(screen.getByRole('textbox', { name: '新容量' })).toHaveValue(
      '10Gi'
    );
  });
  ...
  it('shows a validation error when the size field is cleared', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: '调整容量' }));

    expect(await screen.findByText('请输入新容量')).toBeVisible();
  });
  ...
  it('calls onDismiss after successful submission', async () => {
    ...
    await screen.findByRole('textbox', { name: '新容量' });
