...
describe('EnvironmentHeader', () => {
  ...
  it('renders all status segments with counts', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole('radio', { name: /filter by 在线/i })
      ).toBeVisible();
    });

    expect(
      screen.getByRole('radio', { name: /filter by 总计/i })
    ).toBeVisible();
    expect(
      screen.getByRole('radio', { name: /filter by 离线/i })
    ).toBeVisible();
    expect(
      screen.getByRole('radio', { name: /filter by 未分组/i })
    ).toBeVisible();
  });
  ...
  it('navigates with correct params when Down segment is clicked', async () => {
    ...
    await user.click(screen.getByRole('radio', { name: /filter by 离线/i }));
