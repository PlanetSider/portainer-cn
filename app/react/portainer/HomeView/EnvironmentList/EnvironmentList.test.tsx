...
test('when no environments for query should show empty list message', async () => {
  ...
  await expect(findByText('没有可用的环境。')).resolves.toBeVisible();
});
...
test('sort buttons are rendered for each sort option', async () => {
  ...
  await expect(findByRole('button', { name: /分组/ })).resolves.toBeVisible();
  await expect(
    screen.findByRole('button', { name: /平台/ })
  ).resolves.toBeVisible();
  await expect(
    screen.findByRole('button', { name: /健康状态/ })
  ).resolves.toBeVisible();
});
...
test('clicking inactive sort button switches active sort', async () => {
  ...
  const platformBtn = await screen.findByRole('button', { name: /平台/ });
...
test('Health filter shows only environments matching the selected status', async () => {
  ...
  const healthBtn = await screen.findByRole('button', { name: /健康状态/ });
