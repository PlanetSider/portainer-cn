  ...
  return (
    <Modal aria-label="Kubeconfig 视图" onDismiss={onClose} size="xl">
      ...
      <Modal.Body>
        <div>
          <span>
            请选择要加入 Kubeconfig 文件的 Kubernetes 环境。你可以跨多个页面进行选择。
          </span>
...
export function expiryMessage(expiry: string) {
  switch (expiry) {
    case '24h':
      return 'Kubeconfig 文件将在 1 天后过期。';
    case '168h':
      return 'Kubeconfig 文件将在 7 天后过期。';
    case '720h':
      return 'Kubeconfig 文件将在 30 天后过期。';
    case '8640h':
      return 'Kubeconfig 文件将在 1 年后过期。';
    case '0':
    default:
