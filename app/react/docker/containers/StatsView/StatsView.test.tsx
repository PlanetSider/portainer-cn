...
describe('StatsView', () => {
  it('renders the page header "容器统计信息"', () => {
    renderComponent();

    expect(screen.getByText('容器统计信息')).toBeInTheDocument();
  });

  it('renders the refresh rate select with the correct options', () => {
    renderComponent();

    const select = screen.getByRole('combobox', { name: '刷新频率' });
  ...
  it('shows "无法获取容器统计信息" error panel when stats fetch returns 500', async () => {
    server.use(
      http.get('/api/endpoints/1/docker/containers/container1/stats', () =>
    ...
    await waitFor(() => {
      expect(
        screen.getByText('无法获取容器统计信息')
      ).toBeInTheDocument();
    });
  });

  it('shows the unavailable network message when stats have empty networks', async () => {
    server.use(
      http.get('/api/endpoints/1/docker/containers/container1/stats', () =>
    ...
    await waitFor(() => {
      expect(
        screen.getByText('此容器的网络统计信息不可用。')
      ).toBeInTheDocument();
    });
  });
  ...
  it('does not render the Network chart widget when networkUnavailable is true', async () => {
    ...
    expect(
      screen.queryByText('网络使用情况（汇总）')
    ).not.toBeInTheDocument();
  });

  it('shows the unavailable I/O message when stats have blkio_stats: undefined', async () => {
    server.use(
      http.get('/api/endpoints/1/docker/containers/container1/stats', () =>
    ...
    await waitFor(() => {
      expect(
        screen.getByText('此容器的 I/O 统计信息不可用。')
      ).toBeInTheDocument();
    });
  });
});
